...
class Block:
    def __init__(self, height: int, prev_hash: str, validator: int, timestamp: float):
        self.height = height
        self.prev_hash = prev_hash
        self.validator = validator  # ID of the validator who created this block
        self.timestamp = timestamp
        self.transactions: List[Transaction] = []
        self.votes: Set[int] = set()  # Set of validators who have voted for this block
...
class Peer:
    ...
    def __init__(self, peer_id: int, stake: float, message_bus: MessageBus):
        ...
        self.votes_cast: Dict[int, Set[str]] = {}  # height -> Set of block identifiers we've voted for
    ...
    def process_messages(self):
        ...
        for msg in messages:
            try:
                ...
                print(f"Error processing message {msg['type']} from {msg['sender']}: {e}")
    ...
    def propose_block(self, height: int):
        ...
        block_data = {
            ...
            "transactions": [], # We're simplifying - in a real system we'd include these
    ...
    def receive_block_proposal(self, block_data: Dict, sender_id: int):
        ...
        if height != self.finalized_height + 1:
            if DEBUG:
                print(f"[Peer {self.id}] Rejected block at height {height} (expected {self.finalized_height + 1})")
            return
        
        # Verify the previous hash
        if prev_hash != self.current_hash:
            if DEBUG:
                print(f"[Peer {self.id}] Rejected block with prev_hash {prev_hash} (expected {self.current_hash})")
            return
        
        # Create a block from the received data
        block = Block(height, prev_hash, validator, timestamp)
    ...
    def vote_for_block(self, block: Block):
        ...
        if DEBUG:
            print(f"[Peer {self.id}] Voting for block at height {block.height} by validator {block.validator}")
    ...
    def receive_vote(self, vote_data: Dict, sender_id: int):
        ...
        if block_id in self.pending_blocks:
            ...
            if voter not in block.votes:
                block.votes.add(voter)
                if DEBUG:
                    print(f"[Peer {self.id}] Received vote from peer {voter} for block at height {height}")
    ...
    def check_finalization(self, block_id: str):
        ...
        voter_stake = sum(simulator.peers[voter_id].stake for voter_id in block.votes 
        ...
        if voter_stake >= threshold:
            if DEBUG:
                print(f"[Peer {self.id}] Block at height {block.height} has {len(block.votes)} votes with {voter_stake:.2f}/{total_stake:.2f} stake (threshold: {threshold:.2f})")
            self.finalize_block(block)
        elif DEBUG and random.random() < 0.05:  # Occasionally print vote status
            print(f"[Peer {self.id}] Block at height {block.height} has {len(block.votes)} votes with {voter_stake:.2f}/{total_stake:.2f} stake (need {threshold:.2f})")
    ...
    def print_results(self):
        ...
        if self.peers[0].blockchain:
            ...
            for proposer_id, count in sorted(validator_counts.items(), key=lambda x: x[1], reverse=True):
                ...
                print(f"Peer {proposer_id}: {count} blocks ({blocks_percent:.1f}%) - Stake: {stake_percent:.1f}%")
